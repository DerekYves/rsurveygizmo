...
#' @export
pullsg_campaign <- function(surveyid, api, reset_row_names=T, customfields=T, contactinfo=T, small=F, verbose=T) {
	...
	for(i in 1:cam_respnum){
		...
		if(verbose) message("Retrieving campaign summaries, page ", i)
	...
	cam_dat   <- cam_fullset[ , c("id", "inviteid", "_type", "_subtype", "status", "name", "datecreated")]
	...
	for(i in 1:length(email_ids)){
		...
		for(j in 1:cam_respnum){
			...
			if(verbose) message('Retrieving contact data for "',  camp_name[j], '": page ', j)
	...
	if(customfields==F) contact_fullset <- contact_fullset[ , -grep("^scustomfield", names(contact_fullset))]
	if(contactinfo==F) contact_fullset <- contact_fullset[ , -grep("phone$|^smailingaddress", names(contact_fullset))]
	if(small==T) contact_fullset <- contact_fullset[ , c("contactid", "semailaddress")]
